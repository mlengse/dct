...
<script>
const exceljs = async () => await import('exceljs/dist/es5/exceljs.browser')
const fileSaver = async () => await import('file-saver');
export default {
  props: ['data', 'fields', 'name', 'label', 'loaded'],
  methods: {
    async exportXLSX() {
      if (process.browser) {
        const { Workbook } = await exceljs();
